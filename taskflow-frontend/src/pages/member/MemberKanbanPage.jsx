...
const MemberKanbanPage = () => {
  ...
  const fetchMyTasks = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to load your Kanban tasks');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', height: 'calc(100vh - 120px)' }}>
      ...
      <div>
        ...
        <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.85rem', color: '#666666' }}>
          Visual task board showing your active engineering and design assignments.
        </p>
      </div>
                ...
                {colTasks.length === 0 ? (
                  ...
                  colTasks.map((t) => (
                    ...
                    >
                      ...
                      {/* Movement Buttons */}
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.25rem', borderTop: '1px solid #f0f0f0', paddingTop: '0.5rem' }}>
                        {col.id !== 'TODO' && (
                          <button
                            title="Move back"
                            onClick={(e) => handleMoveStatus(e, t, 'prev')}
                            style={{ padding: '0.2rem 0.45rem', border: '1px solid #d4d4d4', backgroundColor: '#ffffff', borderRadius: '4px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                          >
                            <ChevronLeft size={12} />
                          </button>
                        )}
